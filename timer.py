...
app.layout = dbc.Container(
    [
        dbc.Row(
            [
                dbc.Col(dbc.Label("Client time is:"), width=2),
                dbc.Col(html.H2("", id="client-time"), width=10),
            ]
        ),
        dbc.Row(
            [
                dbc.Col(dbc.Label("Server time is:"), width=2),
...
@app.callback(
    dash.dependencies.Output("server-time", "children"),
...
app.clientside_callback(
    dash.dependencies.ClientsideFunction(
        namespace="clientside", function_name="update_timer"
    ),
