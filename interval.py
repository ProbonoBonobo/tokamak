...
def render(appstate=appstate):
    return [
        dbc.Input(id="foo", value=appstate["foo"]),
...
@app.callback(Output("root", "children"), [Input("interval", "n_intervals")])
def update_metrics(n):
    return render()


@app.callback(
    Output("status", "children"), [Input("foo", "value"), Input("bar", "value")]
)
def update(foo, bar):
    ...
    appstate["bar"] = bar
    return [
        html.P(f"Last updated at: {datetime.datetime.now().isoformat()}"),
