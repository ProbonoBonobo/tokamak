    ...
    def index(self, node_or_id: (Component, str)):
        try:
            if isinstance(node_or_id, str):  # it's an id
                ...
                _ord = list(
                    map(
                        lambda item, node=_node: 1 if item is node else 0,
                        self._node_seq,
                    )
                ).index(1)
        except Exception as e:
            raise IndexError(
                f"Invalid index: {node_or_id} raised {e.__class__.__name} :: {e}"
            )
...
class Reactor(object):
        ...
        if hasattr(self, name):
            ...
            if callable(transformer):
                ...
                if not has_generic_signature:
                    args, kwargs = (
                        [],
                        dict(zip(inspect.getfullargspec(transformer).args[1:], [node])),
                    )
                else:
                    args, kwargs = [node], {}
    ...
    def _generate_callbacks(self):
        def _update_state(*args, targets=None, app_state=None, DEBUG=True, modified=None):
            if (datetime.datetime.now() - self._last_update).microseconds < 100000:
                return no_update

            # print('Updating')
            has_update = False
            """ kz change: update is not firing for some reason, I will comment this out and hope it fixes something"""
            for target, value in zip(targets, args):
                id, prop = target
                curr = getattr(app_state.nodes[id], prop)
                if curr != value:
                    has_update = True
                    self.submit_update(id, prop, value)

                    print(f"Target {target} has changed to {value} (was {curr})")
            if not has_update:
                updated_view = no_update
            else:
                self._last_update = datetime.datetime.now()
                has_update, updated_view = self.update_nodes()
            # try:
        ...
        def batched_callback(*args):
            ...
            # print(context)
            changes = context.triggered
            for change in changes:
                print(f"Change: {change}")
                # if change['prop_id'] == 'submit' and 'n_clicks' in change:
                #     print(f"Yup it actually changed")
                    # self.last_click = change['n_clicks']

            _scoped = partial(
                _update_state, targets=self.targets, app_state=self, DEBUG=self.DEBUG, modified=context.triggered
            )
            return _scoped(*args)
        ...
        self.callback_dependencies = (
            ...
            [
                dash.dependencies.Input(*self.targets[0])
            ],
            [
                dash.dependencies.State(*target)
    ...
    def update_nodes(self):
        ...
        for id, node in self.nodes.items():
            component_updates = [
                (k, v[-1]) for k, v in self.pending[id].items() if len(v)
            ]
                ...
                if self.semaphores[id].acquire():
                    ...
                    for prop, updates in self.pending[id].items():
                        ...
                        try:
                            ...
                            print(cyan("Set {node}.{prop} to {next} (was: {prev})"))
    ...
    def render(self, node):
        ...
        assert (
            callable(self.renderers[node.id]),
            f"self.renderers[{node.id}] maps to a non-functional value: {self.renderers[node.id]} (Type: {type(self.renderers[node.id]).__name__})",
        )
