...
class PosixFilepath:
    def __init__(self, user, path, curr_dir=None):
        ...
        self.has_owner_rwx = (
            self.is_owner & self.stat.S_IRWXU & self.mode == self.stat.S_IRWXU
        )
        self.has_group_rwx = (
            self.is_group & self.stat.S_IRWXG & self.mode == stat.S_IRWXG
        )
        ...
        self.credentials = (
            "owner"
            if self.has_owner_rwx
            else "group"
            if self.has_group_rwx
            else "world"
            if self.has_world_rwx
            else ""
        )
        if curr_dir:
            os.chdir(prev_dir)
