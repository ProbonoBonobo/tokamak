...
class TodoApp(dbc.Jumbotron):
    ...
    @property
    def children(self):
        return [html.Section(children=str(self.context)),
                             html.Section(className='todoapp',
                            children = [html.Header(dbc.Input(id='add-todo', className='new-todo', placeholder='What do you want to do today?'), className='header'),

                             html.Section(html.Div(self.items), className='main'),
                             html.Footer([html.Span(className='todo-count'),
                                          html.Ul([TodoFilter(children='All', id='filter1', key='0'),
                                                   TodoFilter(children='Active', id='filter2', key='1')], className='filters')])])]

app.layout = html.Div(children=TodoApp(), id='root')

@app.callback(dash.dependencies.Output('root', 'children'),
            [dash.dependencies.Input('add-todo', 'n_submit'), dash.dependencies.Input('filter1', 'n_clicks'), dash.dependencies.Input('filter2', 'n_clicks')],
...
def callback(_,selected_filter1, selected_filter2, value):
    ...
    d = {k:getattr(dash.callback_context, k) for k in ('inputs', 'triggered', 'states')}


    out = TodoApp(context=str(d))

    new_state = dict((tuple(tup[0].split(".")), tup[1]) for tup in [tuple(x.values()) for x in dash.callback_context.triggered])
    if value:
        out.add(value)
    print(new_state)
    ...
    if all(TodoFilter(id=id.split(".")[0]).n_clicks == val for id, val in changed_ids.items()):
